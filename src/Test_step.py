...
def train(dataloader, decoder, device, steps=NB_STEPS,base_name="ckpt"):
    ...
    criterion = Hierarchical_Loss(balance=0.5).to(device)
...
if __name__ == "__main__":
    ...
    for seed in [0,1,2]:
            set_seed(seed)
            print(f"\n===== RUN seed={seed} balance=0.5 =====")
    
            dl_card = get_Card_Dataloader_grouped(batch_size=BATCH_SZ, split="train", num_workers=0)
            dl_will = get_Willet_Dataloader_grouped(batch_size=BATCH_SZ, split="train", num_workers=0)
            mixed = MixedBatchIterator(loaders={"card": dl_card, "willett": dl_will}, strategy="alternate")
    
            model = train(mixed, decoder, device, steps=NB_STEPS,base_name=f"bal0p5_seed{seed}")
    
            save_checkpoint(model, None, step=NB_STEPS, extra={"seed": seed}, base_name=f"final_bal0p5_seed{seed}")
